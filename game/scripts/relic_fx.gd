...
var temps: Array = []      # 限时修正 {stat, value, until, key}：同一藏品再次触发只刷新时长，不叠乘
...
func tick(dt: float) -> void:
	...
	if rule("tree_light") > 0:
		...
		if no_hurt_t >= 60.0:
			no_hurt_t = 0.0
			g.lamp = minf(g.lamp_cap, g.lamp + 15.0)
	...
	if dot_mult > 0.0:
		...
		if dot_tick <= 0.0:
			dot_tick = 0.5
			# 按敌人生命的时间倍率缩放（与岁怒 / 净尘一致）：固定 18 到 8:00 只剩开局的 1/5，而童趣玩偶 7:00 后才出现
			var per: float = 18.0 * (g.ch.u_dmg_mult if "u_dmg_mult" in g.ch else 1.0) * g.dmg_mult * dot_mult * 0.5 * g.combat.enemy_hp_time_mult()
			g.combat.hit("藏品")
			for e in g.enemies:
...
func on_skill_start(o = null, i := -1) -> void:
	if g.relics.has("110"):
		_temp("dmg", 0.6, 3.0, "110")
	if g.relics.has("111"):
		_temp("dmg", 1.0, 1.0, "111")
...
## 挨打多的玩家闪避几次就是 ×2.3ⁿ（2026-09-26 纸面核算发现）
func _temp(stat: String, value: float, dur: float, key := "") -> void:
	if key != "":
		for x in temps:
			if x.get("key", "") == key:
				x.until = maxf(x.until, g.t + dur)
				return
	temps.append({"stat": stat, "value": value, "until": g.t + dur, "key": key})
