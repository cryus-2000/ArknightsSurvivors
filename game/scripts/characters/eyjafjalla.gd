...
func update(dt: float) -> void:
	...
	if erupt > 0:
		...
		if erupt_t <= 0.0:
			erupt_t = 0.2
			erupt -= 1
			var c: Vector2 = densest_point((base("range", 290.0) + 60.0) * stat(&"op_range"), pos)
			if c == Vector2.INF:
				var ts: Array = nearest_enemies(1, (base("range", 290.0) + 60.0) * stat(&"op_range"), pos)
				if ts.is_empty():
					erupt = 0
					return
				c = ts[0].pos
...
func _extra_targets(first: Dictionary, n: int) -> Array:
	var out: Array = []
	for e in nearest_enemies(n + 3, (base("range", 290.0) + 40.0) * stat(&"op_range"), pos):
